#!/usr/bin/env node
/**
 * GameHub AI自动化工作流主程序 (CommonJS版本)
 * 
 * 功能：
 * - 定时检查游戏更新
 * - 自动生成攻略内容
...
const { runContentMonitor } = require('./content-monitor.cjs');
const { runCodeChecker } = require('./code-checker.cjs');
const { runGameUpdater } = require('./game-updater.cjs');
...
用法:
  node scripts/ai-workflow.cjs [选项]
