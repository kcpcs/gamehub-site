...
async function runTests() {
  ...
  await testEndpoint('Games List (with limit)', '/api/games?limit=5')
  ...
  // Sitemap & Robots
  await testEndpoint('Sitemap XML', '/sitemap.xml')
  ...
  console.log(`\n⏱️  Average response time: ${Math.round(results.filter(r => r.time).reduce((sum, r) => sum + (r.time || 0), 0) / passed)}ms`)
