...
export function GamesClient({ initialGames }: GamesClientProps) {
  ...
  const [isInitialLoad, setIsInitialLoad] = useState(initialGames.length === 0)
  ...
  const filteredGames = games.filter((game) => {
    if (filters.platform && !game.platforms.includes(filters.platform as Platform)) return false
    if (filters.genre && !game.genres.includes(filters.genre as Genre)) return false
      ...
      {filteredGames.length === 0 && (
        <div className="text-center py-16" style={{ color: 'var(--text-muted)' }}>
          <svg className="w-16 h-16 mx-auto mb-4 opacity-50" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M9.172 16.172a4 4 0 015.656 0M9 10h.01M15 10h.01M21 12a9 9 0 11-18 0a9 9 0 0118 0z" />
          </svg>
