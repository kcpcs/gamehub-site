...
export default function TierListPage({ params }: { params: Promise<{ game: string }> }) {
  ...
  useEffect(() => {
    const fetchTierList = async () => {
      try {
        const res = await fetch(`/api/tierlist/${game}?category=${activeCategory}&version=${activeVersion}`)
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-8">
      ...
      <nav className="flex items-center gap-2 text-sm mb-6" style={{ color: 'var(--text-secondary)' }}>
        ...
        <span>/</span>
        <a href={`/games/${tierList.game_slug}`} className="hover:text-[var(--accent-light)]">
          {tierList.game_name}
        </a>
        <span>/</span>
        <span>Tier List</span>
